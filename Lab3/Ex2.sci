function [yn, yorigin] = advance(xn, xorigin, k)
    // 1. Tính toán ngõ ra
    yn = xn;
    yorigin = xorigin + k; // Advance thì ta + k (với k > 0)

    // 2. Tạo trục thời gian n cho x(n) và y(n)
    L = length(xn);
    nx = (1:L) - xorigin; 
    ny = (1:L) - yorigin; 

    // Bước tìm giới hạn chung giữa các cột nx, ny, xn, yn để căn đều cho hai đồ thị
    ...
    // --- Vẽ đồ thị Signal Advance y(n) = x(n + k) ---
    subplot(2,1,2);
    plot2d3(ny, yn); 
