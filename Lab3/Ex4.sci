function [yn, yorigin] = add(x1n, x1origin, x2n, x2origin)
    ...
    // khoi tao hai mang chua toan so 0 co cung chieu dai voi truc chung de dong bo
    y1 = zeros(1, length(ny));
    y2 = zeros(1, length(ny));

    // anh xa du lieu tu mang goc sang mang chung dung vi tri
    y1( (n1(1) - n_min + 1) : (n1($) - n_min + 1) ) = x1n;
    y2( (n2(1) - n_min + 1) : (n2($) - n_min + 1) ) = x2n;

    // cong hai mang da duoc dong bo de tao ra ngõ ra
    yn = y1 + y2;

    // tinh vi tri goc thoi gian moi
    // diem n bang 0 se nam o vi tri 1 tru cho n_min
    yorigin = 1 - n_min;

    // tim gioi han de ve do thi can deu ca 3 hinh
    v_min = min(min(yn), 0);
